"""
Build and Export GR86 model to GLTF/GLB for web
"""
...
def create_wheel(name, x, y, z):
    bpy.ops.mesh.primitive_torus_add(major_radius=0.35, minor_radius=0.15, location=(x, y, z))
    ...
    bpy.ops.mesh.primitive_cylinder_add(radius=0.25, depth=0.1, location=(x, y + (0.16 if y > 0 else -0.16), z))
...
def create_lights():
    for y in [WIDTH * 0.15, -WIDTH * 0.15]:
        bpy.ops.mesh.primitive_uv_sphere_add(radius=0.1, location=(LENGTH * 0.45, y, HEIGHT * 0.25))
    ...
    for y in [WIDTH * 0.15, -WIDTH * 0.15]:
        bpy.ops.mesh.primitive_cube_add(size=0.1, location=(-LENGTH * 0.45, y, HEIGHT * 0.25))
...
def create_grille():
    bpy.ops.mesh.primitive_cube_add(size=1, location=(LENGTH * 0.45, 0, HEIGHT * 0.18))
...
def create_windshield():
    bpy.ops.mesh.primitive_cube_add(size=1, location=(LENGTH * 0.18, 0, HEIGHT * 0.5))
    ...
    bpy.ops.mesh.primitive_cube_add(size=1, location=(-LENGTH * 0.12, 0, HEIGHT * 0.5))
...
create_hood()
create_trunk()
create_wheel("FL_Wheel", LENGTH * 0.28, WIDTH * 0.22, HEIGHT * 0.35)
create_wheel("FR_Wheel", LENGTH * 0.28, -WIDTH * 0.22, HEIGHT * 0.35)
create_wheel("RL_Wheel", -LENGTH * 0.28, WIDTH * 0.22, HEIGHT * 0.35)
create_wheel("RR_Wheel", -LENGTH * 0.28, -WIDTH * 0.22, HEIGHT * 0.35)
create_lights()
create_grille()
create_windshield()
...
print(f"Export complete! File size: {os.path.getsize(output_file)} bytes")
